...
# Lower = faster feel. 0.15s baseline matches a comfortable mobile action pace.
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == KEY_F2:
			...
			get_node("/root/CombatLog").post(label + " 모드 전환", Color(0.6, 1.0, 0.5))
			# Reset all monster pending_energy so no burst of actions happens on switch.
...
func _tick_monsters(delta: float, tm: Node, player: Player) -> void:
	# Drain abort flag before loop in case a turn-based end left it set.
