...
### 1a. 11 files — completely clean (direct copy, no edits)
...
| `ResultScreen.gd` + `.tscn` | End-of-run screen (victory / death with stats). One line calling `GameManager.selected_job_id/race_id` — swap to plain `start_new_run()` or add those fields to your GameManager. |
| `BottomHUD.gd` + `.tscn` | Action bar — quickslot row + menu buttons. |
| `TopHUD.gd` + `.tscn` | HP/MP bars + location label + minimap thumbnail. One comment mentions `BranchRegistry`; delete the comment, code itself is clean. |
| `QuickSlot.gd` + `.tscn` | Single quickslot button widget. |
| `SkillLevelUpToast.gd` + `.tscn` | Pop-up toast when a skill levels up. |
...
**Correction (2026-04-22 session)** — two files previously listed here are **NOT clean** and have moved to §1h:
- `BagTooltips.gd`: heavy coupling to DCSS-derived `WeaponRegistry`, `ArmorRegistry`, `RingRegistry`, `ConsumableRegistry`, `WandRegistry`, `FormRegistry`, `SpellRegistry` (20+ call sites). Rewrite from scratch against your new ItemData.
- `SkillsScreen.gd`: tight coupling to DCSS-port `SkillSystem` class (imports `SkillSystem.SKILL_IDS`, `SkillSystem.SKILL_CATEGORY`, signals). Rewrite when your SkillSystem lands.
