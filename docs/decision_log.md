...
Verification status: `godot --headless --check-only` passes (EXIT 0). F5 smoke verification pending — user will run end-to-end on next session. Deferred items: town hub, turn budget, fixed authored 42×47 maps, character minimap memory, essence slot 1/6/14 unlock progression, starter shop, job/class system removal, combat formula d100 rewrite, balance pass (XP rates, hidden-tier 20% contribution, the four new skills' XP sources).
