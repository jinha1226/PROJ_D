...
- [x] **Rune / Orb endgame** — session 9 (74b34dbd): RuneRegistry with 15 runes + ZOT_GATE_REQUIREMENT=3. Player.runes[] + has_orb. GameBootstrap._maybe_place_rune_and_orb drops the branch rune on the final floor (once per floor per run, guarded by GameManager.runes_placed), Orb on Zot:5. Zot entrance gated by 3 runes; D:1 stairs-up with Orb wins the run. Status dialog shows collection.
- [x] **Cloud system** — session 9: CloudSystem.gd pure-data module with 6 types (fire/freezing/mephitic/noxious/smoke/ice). GameManager.clouds dict keyed by Vector2i; cleared on floor regen + new run. _on_turn_tick_clouds connects to player_turn_started, applies damage/status then decrements turns_left. DungeonMap renders translucent per-type overlay (visibility-gated, DCSS behaviour). Smoke clouds block FOV via _opaque_at → CloudSystem.blocks_fov. Fireball + Fire Storm + Hailstorm leave cloud residue via _cast_area_spell → _spell_cloud_residue. Monster-breath cloud spawning still TBD (no dragon breath mob spell wired yet).
